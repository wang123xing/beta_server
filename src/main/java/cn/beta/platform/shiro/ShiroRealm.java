...
import cn.beta.platform.utils.SpringContextUtils;
...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        String mobile = null;
        Integer loginType = LoginTypeEnum.PLATFORM.getValue();
        if (principals != null) {
            LoginUserDTO sysUser = (LoginUserDTO) principals.getPrimaryPrincipal();
            mobile = sysUser.getUsername();
        }
        SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
        // 设置用户拥有的角色集合，比如“admin,test”
        Set<String> roleSet = null; // todo 是否需要权限
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
        ...
        // 校验token有效性
        LoginUserDTO loginUser = this.checkUserTokenIsEffect(token);
    ...
    public LoginUserDTO checkUserTokenIsEffect(String token) throws AuthenticationException {
        ...
        LoginUserDTO loginUser = userService.getUserByLoginName(jwtMessage);
        // 查询用户信息
        log.debug("———校验token是否有效————checkUserTokenIsEffect——————— "+ token);
        if (loginUser == null) {
            throw new AuthenticationException("用户不存在!");
        }
        if (loginUser.getStatus()== CommonAllowEnum.DISABLE.getValue()) {
            throw new AuthenticationException("用户被禁用，请联系管理员!");
        }
        // 判断用户状态
        // 校验token是否超时失效 & 或者账号密码是否错误
        if (!jwtTokenRefresh(token, loginUser.getUsername(), loginUser.getPassword())) {
